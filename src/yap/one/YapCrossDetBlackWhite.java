...
public class YapCrossDetBlackWhite {
	...
	private static boolean[] blackRows;
	private static boolean[] whiteRows;
	private static boolean[] blackColumns;
	private static boolean[] whiteColumns;
...
//		drawMatrix("output_images/image_det.png");
	...
	private static void setBlackWhiteColumns() {
		for (int j = 0; j < M; ++j) {
			blackColumns = new boolean[N];
			whiteColumns = new boolean[N];
			getBlackWhiteColumns(j, 0, 0, freedomsUp[j]);
			for (int i = 0; i < N; ++i) {
				if (!blackColumns[i]) {
					MATRIX[i][j] = -1;
				} else if (!whiteColumns[i]) {
					MATRIX[i][j] = 1;
				}
			}
		}
	...
	private static void setBlackWhiteRows() {
		for (int i = 0; i < N; ++i) {
			blackRows = new boolean[M];
			whiteRows = new boolean[M];
			getBlackWhiteRows(i, 0, 0, freedomsLeft[i]);
			for (int j = 0; j < M; ++j) {
				if (!blackRows[j]) {
					MATRIX[i][j] = -1;
				} else if (!whiteRows[j]) {
	...
	private static boolean getBlackWhiteRows(int i, int valueIndex, int startJ, int f) {
		List<Integer> list = LEFT.get(i);
		int value = list.get(valueIndex);
		for (int j = startJ; j <= startJ + f; ++j) {
			int nextIndex = j + value;
			...
			if (valueIndex < list.size() - 1 && getBlackWhiteRows(i, valueIndex + 1, nextIndex + 1, f - (j - startJ))) {
				return true;
			}
			if (isRowValid(i)) {
				for (int k = 0; k < M; ++k) {
	...
	private static boolean getBlackWhiteColumns(int j, int valueIndex, int startI, int f) {
		List<Integer> list = UP.get(j);
		int value = list.get(valueIndex);
		for (int i = startI; i <= startI + f; ++i) {
			int nextIndex = i + value;
			...
			if (valueIndex < list.size() - 1
					&& getBlackWhiteColumns(j, valueIndex + 1, nextIndex + 1, f - (i - startI))) {
				return true;
			}
			if (isColumnValid(j)) {
				for (int k = 0; k < N; ++k) {
